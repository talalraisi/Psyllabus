...
export const DIFFICULTY_POINTS = { easy: 0.5, medium: 0.75, hard: 1.25 }
...
/* ---------------------------------------------------------------------------
   ...
   Three bands are enough to score a question and not enough to describe one.
   The distance between a standard Paper 1 question and the last part of the
   hardest question on Paper 2 is most of what makes a subject hard, and
   calling both of them "hard" hides exactly the thing a student is trying to
   find when they go looking for a challenge.

   So five tiers for reading, still monotonic with the same difficulty number
   the bands use. Low, Medium and Extreme keep the point values already set;
   Hot and Burning fill the gaps that were being rounded away. Burning is meant
   to be rare: the questions that separate a 6 from a 7.
   --------------------------------------------------------------------------- */
...
export const HEAT_LEVELS = [
  {
    ...
    max: 0.3,
    points: 0.5,
    description: 'Recall and one step. You should get these every time.',
  },
  {
    key: 'medium',
    label: 'Medium',
    max: 0.5,
    points: 0.75,
    description: 'A couple of steps, or a definition applied to something new.',
  },
  {
    key: 'hot',
    label: 'Hot',
    max: 0.68,
    points: 1,
    description: 'Multi-step, and it is not obvious which method to start with.',
  },
  {
    key: 'extreme',
    label: 'Extremely hot',
    max: 0.82,
    points: 1.25,
    description: 'Combines topics, or hides the thing being tested.',
  },
  {
    key: 'burning',
    label: 'Burning',
    max: Infinity,
    points: 1.5,
    description: 'The hardest end of the paper. These are the ones that separate grades.',
  },
]

/** key → [min, max], for filtering a pool by heat. Min is exclusive. */
export const HEAT_RANGES = HEAT_LEVELS.reduce((acc, level, i) => {
  acc[level.key] = [i === 0 ? 0 : HEAT_LEVELS[i - 1].max, level.max === Infinity ? 1 : level.max]
  return acc
}, {})

export function heatOf(difficulty) {
  const d = typeof difficulty === 'number' ? difficulty : 0.5
  return HEAT_LEVELS.find((h) => d <= h.max) ?? HEAT_LEVELS[HEAT_LEVELS.length - 1]
}

export const HEAT_TEXT_COLORS = {
  low: 'text-[var(--heat-low)]',
  medium: 'text-[var(--heat-medium)]',
  hot: 'text-[var(--heat-hot)]',
  extreme: 'text-[var(--heat-extreme)]',
  burning: 'text-[var(--heat-burning)]',
}

export const HEAT_BG_COLORS = {
  low: 'bg-[var(--heat-low)]',
  medium: 'bg-[var(--heat-medium)]',
  hot: 'bg-[var(--heat-hot)]',
  extreme: 'bg-[var(--heat-extreme)]',
  burning: 'bg-[var(--heat-burning)]',
}

/**
 * Points follow heat rather than the three bands, so a Burning question is
 * worth more than an Extremely hot one instead of both being "hard". The
 * numbers already chosen for easy, medium and hard are unchanged at Low,
 * Medium and Extremely hot; Hot and Burning are the two new values.
 */
export function pointsForQuestion(difficulty) {
  return heatOf(difficulty).points
}
...
export const POINT_BANDS = [
  { min: 9, status: 'mastered' },
  { min: 7, status: 'proficient' },
  { min: 5, status: 'confident' },
...
/**
 * A subtopic title as it should be shown.
 *
 * Several sheets write the tier into the title, and hl_only already drives a
 * badge next to it, so the label came out as "... (HL only)  HL only". The
 * badge is the right place for it; the title should just be the title.
 */
export function displaySubtopic(subtopic) {
  return String(subtopic || '')
    .replace(/\s*[([]?\s*(HL|SL|AS|A2|Higher|Standard)\s+only\s*[)\]]?\s*$/i, '')
    .replace(/\s{2,}/g, ' ')
    .trim()
}
...
/**
 * The guide's order, when we have it.
 *
 * Sorting a course by name puts momentum before kinematics and complex numbers
 * before indices, which is not how anybody is taught it. Rows carry a position
 * from the syllabus outline; older two-level maps do not, and fall back to the
 * topic number as before.
 */
const positionOf = (items) =>
  items.reduce((min, i) => (i.position == null ? min : Math.min(min, i.position)), Infinity)

export function sortTopics(entries) {
  return entries.sort(([a, aItems], [b, bItems]) => {
    const pa = positionOf(aItems || [])
    const pb = positionOf(bItems || [])
    if (pa !== pb) return pa - pb
    return topicSortKey(a) - topicSortKey(b)
  })
}

/** Syllabus rows in the order the guide lists them. */
export function orderSyllabus(items) {
  return [...items].sort(
    (a, b) =>
...
/**
 * The middle level: a topic's numbered units, each holding its subtopics.
 *
 * Returns a single unnamed group for the curricula whose maps are still two
 * levels deep, so a caller can render both without asking which it has.
 */
export function groupByUnit(items) {
  const groups = []
  for (const item of orderSyllabus(items)) {
    const last = groups[groups.length - 1]
    if (last && last.unit === (item.unit || null)) last.items.push(item)
    else groups.push({ unit: item.unit || null, code: item.code || null, items: [item] })
  }
  return groups
}
