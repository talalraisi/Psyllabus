...
/**
 * Bring a card back soon, rather than tomorrow.
 *
 * The boxes are measured in days, which is right for a card you got right
 * and useless for one you just got wrong — the answer is still on the screen
 * and the next honest test of it is in a few minutes, not next week. So a
 * wrong answer asks when to see it again and this records that, without
 * touching the box.
 *
 * The box is deliberately left where `scheduleAfter` put it. Snoozing is
 * about this session; the box is about the weeks after it.
 */
export const SNOOZE_CHOICES = [
  { minutes: 1, label: 'In a minute' },
  { minutes: 5, label: 'In 5 minutes' },
  { minutes: 10, label: 'In 10 minutes' },
  { minutes: 60, label: 'In an hour' },
]

export function snoozeAfter(card, minutes) {
  return {
...
/**
 * Should this card come back before the session ends?
 *
 * Anything asked for within the quarter hour is worth re-queueing now; an
 * hour means "not today", and the session will be over.
 */
export function returnsThisSession(minutes) {
  return minutes <= 10
}
