/**
 * Onboarding names to the names used in the original CSV.
 *
 * Worth knowing before relying on this: syllabus_content actually stores the
 * onboarding names, not the mapped ones, so getDbSubjectName returns strings
 * that are not in the table. Nothing queries with it. It survives only as a
 * fallback inside resolveSubjectFromSlug, for older profiles that might hold a
 * name in the other form.
 */
export const SUBJECT_MAP = {
  ...
  // DP Core. These names must match syllabus_content exactly, otherwise the
  // slug will not resolve back and the syllabus page bounces to the dashboard.
  ...
  // Older labels kept so existing profiles keep working.
...
/**
 * Turn a slug back into a subject the student actually takes.
 *
 * SUBJECT_MAP only ever held IB names, so every A-Level and AP subject failed
 * to resolve and the syllabus page bounced to the dashboard. Matching against
 * the student's own subject list needs no map at all and cannot go stale when
 * a curriculum is added, because the answer is always drawn from the profile
 * in front of us.
 *
 * The IB map is still tried first, since older profiles store onboarding names
 * like "Physics HL" that differ from the database name.
 */
export function resolveSubjectFromSlug(slug, subjects = []) {
  const direct = subjects.find((s) => toSlug(s) === slug)
  if (direct) return direct

  // An IB profile may store a name that maps to a different database name.
  const mapped = resolveOnboardingNameFromSlug(slug)
  if (mapped && subjects.includes(mapped)) return mapped

  const viaMap = subjects.find((s) => toSlug(getDbSubjectName(s) ?? s) === slug)
