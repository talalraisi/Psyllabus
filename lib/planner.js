...
export function scoreItem(
  ...
  // Covered in class and not yet proved is the real backlog: the student has
  // been taught it, so testing it now will tell them something. Something the
  // class has not reached yet is not their fault, and pushing it up the list
  // would just make the plan feel like a rebuke.
  if (detail?.covered && status === 'not_started') {
    score += 28
    reasons.push({ kind: 'taught', label: 'Covered in class, not tested yet' })
  } else if (!detail?.covered && status === 'not_started') {
    score -= 20
    reasons.push({ kind: 'untaught', label: 'Not covered in class yet' })
  }

  // Just reviewed it. Worth less today than something untouched, without
  // pretending the review proved anything.
  const reviewedDaysAgo = detail?.reviewedAt
    ? Math.floor((now - new Date(detail.reviewedAt).getTime()) / 86400000)
    : null
  if (reviewedDaysAgo !== null && reviewedDaysAgo < 3) {
    score -= 18
    reasons.push({ kind: 'reviewed', label: reviewedDaysAgo === 0 ? 'Reviewed today' : `Reviewed ${reviewedDaysAgo}d ago` })
  }
