...
export async function getSyllabus(supabase, subjects = []) {
  ...
  for (const subject of subjects) {
    const entry = read(`psy:syllabus:v2:${subject}`)
  ...
  const { data, error } = await supabase
    ...
    .select('id, subject, topic, unit, code, subtopic, position, hl_only, hl_extension')
    .in('subject', missing)
  if (error) return held

  const bySubject = new Map(missing.map((s) => [s, []]))
  for (const row of data || []) bySubject.get(row.subject)?.push(row)
  for (const [subject, rows] of bySubject) write(`psy:syllabus:v2:${subject}`, rows)
